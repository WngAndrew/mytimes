...
import { LETTERS, potentialWords } from "../word_data/lettersAndWords";
import BackButton from '../../global_components/BackButton.js'; // Import BackButton component

const SOLUTION = "megan"; // Example solution word
...
export default function Wordle() {
 ...
 // Check if Wordle is already completed on component mount
  useEffect(() => {
    const wordleCompleted = JSON.parse(localStorage.getItem("wordleCompleted")) || false;
    if (wordleCompleted) {
      setSolutionFound(true);
      setNotification("Already Completed! 🎉");
    }
    wordleRef.current.focus(); // Keep the component focused for keyboard input
  }, []);

  // Update localStorage when puzzle is completed
  const handleCompletion = () => {
    setSolutionFound(true);
    localStorage.setItem("wordleComplete", JSON.stringify(true));

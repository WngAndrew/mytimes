...
function WordRow({ words }) {
  return (
    <div
      className={`grid grid-cols-4 ${styles.cardsContainer}`}
      style={{
        width: "456px", // Set the exact width
        maxWidth: "456px",
    ...
    >
      {words.map((word) => (
        <WordButton
          ...
          className="w-full h-full" // Make each WordButton fill its cell
        />
      ))}
    </div>
  );
  ...
  return (
    <div>
      {(isGameOverAndWon || isGameActiveWithAnySolvedRows) && (
        <div className="grid gap-2-4 pb-2">
